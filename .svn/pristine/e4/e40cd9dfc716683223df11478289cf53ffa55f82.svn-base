 ...
 * 
 */
public class ExternalLinkRemoval {
	 ...
	 */
	public ExternalLinkRemoval(String transformationIndexFileOrDir,
				...
				while (br.ready()) {
					...
					try {
						...
						for (int i = 2; i < tok.length; i++) {
							...
							// write only if internal link is used
	...
	private static void startMapping(String transformationIndexFileOrDir,
			String networkFileOrDir, String output, long maxIndex, int threads)
			throws IOException {
		ExternalLinkRemoval map = new ExternalLinkRemoval(
