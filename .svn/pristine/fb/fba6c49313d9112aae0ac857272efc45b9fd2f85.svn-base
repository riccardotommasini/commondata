...
classes = FOREACH typeQuads GENERATE object.value as class, de.wbsg.loddesc.functions.PayLevelDomain(graph) AS domain, graph as url;
group_classes_pld = GROUP classes BY (class, domain);
cnt = FOREACH group_classes_pld GENERATE group.class, group.domain, COUNT(classes) as count;
ord = ORDER cnt BY class, count;
STORE ord INTO '$results/classpldocc';