...
-- get urls
typedurls = FOREACH filteredtypedquads GENERATE graph as url, type as type;
distinctedtypedurls = DISTINCT typedurls;
STORE distinctedtypedurls INTO '$results/typedurls' USING PigStorage('$divider');

-- get only information from quads which belong to a type urls
tmp = JOIN distinctedtypedurls BY url, quads by graph;
-- create the quint
...
-- sort the quint
final = ORDER cleantmp BY type, graph, subject;
-- store
